...
interface RawOrderItem {
  ...
  created_at: string
  photo: { id: string; storage_path: string | null; preview_path: string | null; delivered_path: string | null } | null
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RawOrderItem[]> => {
      const { data, error } = await supabase
        .from('order_items')
        .select('*, photo:photos(id, storage_path, preview_path, delivered_path), event:events(title), order:orders(payment_method, created_at, biker:profiles(display_name))')
