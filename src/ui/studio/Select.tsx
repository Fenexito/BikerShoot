...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(({ label, className, id, children, ...props }, ref) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      {label && (
        <label htmlFor={selectId} className="text-xs font-medium uppercase tracking-wider2 text-muted-foreground">
          {label}
        </label>
      )}
      <select
        ...
        id={selectId}
        className={cn(
          'h-12 border border-border bg-input px-4 text-sm text-foreground md:h-14',
